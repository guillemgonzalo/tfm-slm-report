...
VRAM             & 24 GB   & +100\%  & 48 GB   & +100\% & 96 GB   \\ \hline
...
\addplot coordinates {(VRAM (GB),400) (Bus (bits),267) (CUDA cores (×100),324)};
...
\paragraph{L4 → L40S.} El bus se duplica de 192 a \textbf{384 bits} y la VRAM de 24 a \textbf{48 GB}, eliminando los errores OOM con batch sizes mayores. Los núcleos CUDA pasan de 7.424 a \textbf{18.176} (+145\%), incrementando el paralelismo de cómputo de forma sustancial.

\paragraph{L40S → RTX PRO 6000.} La VRAM vuelve a incrementarse hasta \textbf{96 GB} (+100\%), el bus alcanza \textbf{512 bits} y la memoria migra de GDDR6 a \textbf{GDDR7}, con mayor ancho de banda por pin. Los \textbf{24.064 núcleos CUDA} suponen un +32\% sobre la L40S y un +224\% acumulado sobre la L4. Esta configuración permitió alcanzar picos de $\approx$90 GB durante el entrenamiento sin restricciones.
...
A lo largo del proceso de entrenamiento se identificaron y corrigieron nueve limitaciones de la arquitectura inicial mediante un ciclo de desarrollo iterativo: persistencia de estados ocultos GRU entre bloques, reposicionamiento del GRU antes de la atención (GRU → MHA → MLP), inicialización ortogonal de pesos recurrentes, gradient clipping con norma máxima 1.0, logging de contribución por componente, validación periódica contra un subconjunto no visto, una herramienta de análisis arquitectónico (\texttt{HybridArchitectureAnalyzer}), una suite de tests automatizados y la integración de Flash Attention. El análisis detallado de cada mejora, incluyendo su justificación técnica e impacto cuantificado sobre la convergencia, se documenta en el Capítulo~\ref{sec:implementation}.
...
\paragraph{G6 → G6e (L4 → L40S).} Se migró a la instancia \texttt{g6e.2xlarge} equipada con la \textbf{NVIDIA L40S} (48 GB VRAM). El bus de memoria se duplica de 192 a 384 bits y la VRAM pasa de 24 a 48 GB, eliminando los errores OOM que aparecían con configuraciones de batch más agresivas. Esta instancia sirvió de paso intermedio para validar el pipeline completo con mayor holgura de memoria.

\paragraph{G6e → G7e (L40S → RTX PRO 6000).} Con el batch definitivo de 64 muestras (batch efectivo 128 con acumulación de 2), el pico de memoria durante el forward pass de evaluación alcanzó 66,76 GB, superando los 48 GB de la L40S. El entrenamiento completo (incluyendo gradientes y estados del optimizador) llegó a $\approx$90 GB, requiriendo la migración definitiva a la familia \textbf{G7e}, equipada con la \textbf{NVIDIA RTX PRO 6000 Server} (96 GB VRAM, bus 512 bits, memoria GDDR7). Ambas migraciones mantuvieron la misma arquitectura de despliegue basada en Terraform y Docker, sin cambios en el pipeline de entrenamiento.