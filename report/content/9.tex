...
Un pilar fundamental es la \textbf{idempotencia}: cada servicio verifica el estado del sistema antes de ejecutar operaciones costosas, optimizando el uso de recursos en la nube.
...
\begin{enumerate}
    ...
    \item \textbf{Repositorio ECR} (\texttt{tfm-slm}): almacena la imagen Docker con mutabilidad de etiquetas activada y política de ciclo de vida que retiene únicamente la última imagen, minimizando costes de almacenamiento.
...
\begin{figure}[H]
    ...
    \caption{Especificaciones de instancia EC2 g7e.2xlarge: 8 vCPUs (Intel Xeon Emerald Rapids), 64 GB RAM, NVIDIA RTX PRO Server 6000 con 96 GB VRAM. Esta configuración proporciona suficiente memoria para entrenar modelos de 167M parámetros con batch sizes de 64 tokens sin spilling a disco.}
...
\begin{enumerate}
    ...
    \item \textbf{Commit y Push}: Los cambios se registran con \texttt{git commit \&\& git push} en el repositorio Git local (simulado con Git daemon en la máquina del desarrollador).
