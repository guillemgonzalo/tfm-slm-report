...
Esta sección describe el dataset y el pipeline \textbf{tal como se usaron para producir el checkpoint evaluado en este TFM} --- es decir, antes de la corrección metodológica de la Sección~\ref{subsec:mezcla_elastica}. Con la mezcla y ponderación descritas, el dataset consolidado de ese entrenamiento tiene las siguientes características:
...
\quad — reservadas para validación & Ninguna en este entrenamiento: no existía holdout real en el pipeline que generó el checkpoint evaluado (corregido posteriormente, ver Sección~\ref{subsec:mezcla_elastica}) & 0 \\ \hline
...
\end{tabularx}
\caption{Estadísticas reales del dataset del entrenamiento evaluado (verificadas sobre archivos Arrow y código fuente del \textit{trainer}), previo a la corrección metodológica. Ese pipeline no realizaba ninguna partición train/val: el 100\% de las 346.439 muestras se usó en cada una de las 15 épocas. El corpus tiene ~354,8M tokens únicos; al repetirse 15 veces se procesan ~5.321M tokens en total, pero la magnitud relevante para las leyes de escala es la de tokens \textbf{únicos}, no la de tokens procesados con repetición. La Sección~\ref{subsec:mezcla_elastica} describe el pipeline rediseñado, que sí reserva un holdout genuino de 148.437 muestras.}
\end{table}
