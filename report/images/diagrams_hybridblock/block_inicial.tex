...
\definecolor{warncolor}{RGB}{180, 60, 60}
...
\begin{tikzpicture}[node distance=0.55cm]
  ...
  % ── Block title ────────────────────────────────────────────
  \node[font=\footnotesize\itshape, text=gray!60] (title) {Diseño Inicial (MHA $\to$ GRU $\to$ MLP)};
  ...
  \node[grublock,  below=of ln2]             (gru)  {GRU};
  \node[addblock,  below=of gru]             (add2) {$+$};
  ...
  \begin{scope}[on background layer]
    \node[
      ...
      inner sep=10pt,
      fit=(ln1)(mha)(add1)(ln2)(gru)(add2)(ln3)(mlp)(add3)
    ] (border) {};
  \end{scope}

  % ── GRU hidden state annotation (discarded) ─────────────────
  \node[
    font=\scriptsize, text=warncolor,
